...
<details>
  ...
  - On windows:
    1. Download `openocd` from the [xpack project github release](https://github.com/xpack-dev-tools/openocd-xpack/releases). It probably doesn't contains an installer. Simply extract the folder contents somewhere on your harddrive, then add the path to `openocd.exe` (in the `bin` subdirectory) to your system PATH environment variable.
  ...
  - On linux / macOS: 
    1. Download `openocd` from your package manager. (If that's not available, you may also use the [xpack project github release](https://github.com/xpack-dev-tools/openocd-xpack/releases). It probably doesn't contains an installer. Simply extract the folder contents somewhere on your harddrive, then add the path to the `openocd` executable file (in the `bin` subdirectory) to your system search paths).
    2. Download the `*.hex` firmware image file [from the latest release](https://github.com/TheSlowGrowth/Wunderkiste/releases/latest)
